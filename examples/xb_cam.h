...
/*===========================================================================
    xb_cam.h -- Xbox USB Camera interface
                Fully reverse-engineered from Xbox Video Chat XBE (MS-124)

    Sources:
        - Xbox Video Chat XBE (default.xbe) full Ghidra C decompilation
        - xboxkrnl.pdb kernel symbol table
    ...
    Xbox Live Camera  VID=0x045E  PID=0x028C  (OV530 chipset)
    EyeToy (patched)  VID=0x054C  PID=0x0155  (OV519, silicon-compatible)

    Hex patch to accept EyeToy (applied to default.xbe):
        Search:  5E 04 0F 85 CA 01 00 00 66 81 7C 24 1E 8C 02
        Replace: 4C 05 0F 85 CA 01 00 00 66 81 7C 24 1E 55 01
    This swaps the VID/PID comparison in the driver detection path.

    USB: OHCI isochronous, endpoint 1 IN, max 896 bytes/packet (highest alt)
    Channel struct magic: 'CHAN' (0x4348414E) at offset 0

    ═══════════════════════════════════════════════════════════════════════
    ARCHITECTURE
    ═══════════════════════════════════════════════════════════════════════

    The XBE communicates with the usbcamd minidriver entirely through:
        1. NtSetValueKey    - registry writes to configure the driver
        2. NtDeviceIoControlFile - IOCTL dispatch for stream control
        3. NtOpenFile       - opens device handle (path from CameraStatusPath)
    ...
    kernel streaming (KS) filter object. It holds:
        ...
        - USB device handle, EEPROM addr, stream state, frame counters
...
/* ── XDEVICE_TYPE_CAMERA ──────────────────────────────────────────────────── */
/* NOT present in kernel exports or XTL headers. The camera device type is
   likely defined entirely within the video chat XBE. XVoiceCreateMediaObject
   validates against the three known voice tables and will REJECT an unknown
   table pointer. Use the direct IOCTL path (NtDeviceIoControlFile) instead
   of XVoiceCreateMediaObject for camera access.                              */
...
/* ── API declarations (implement in xb_cam.cpp) ───────────────────────────── */
/*
    int  XCam_Init(DWORD dwControllerPort);
    void XCam_Shutdown(void);
    BYTE* XCam_GetFrame(DWORD dwTimeoutMs);
    int  XCam_IsStreaming(void);
    int  XCam_SetFormat(DWORD dwFourCC, DWORD dwWidth, DWORD dwHeight);
*/
